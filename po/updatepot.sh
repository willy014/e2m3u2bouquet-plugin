...
#
printf "Creating temporary file e2m3u2bouquet-py.pot\n"
find $findoptions .. -name "*.py" -exec xgettext --no-wrap -L Python --from-code=UTF-8 -kpgettext:1c,2 --add-comments="TRANSLATORS:" -d e2m3u2bouquet -s -o e2m3u2bouquet-py.pot {} \+
$localgsed --in-place e2m3u2bouquet-py.pot --expression=s/CHARSET/UTF-8/
printf "Creating temporary file e2m3u2bouquet-xml.pot\n"
which python
if [ $? -eq 0 ]; then
	find $findoptions .. -name "*.xml" -exec python xml2po.py {} \+ > e2m3u2bouquet-xml.pot
else
	find $findoptions .. -name "*.xml" -exec python3 xml2po.py {} \+ > e2m3u2bouquet-xml.pot
fi
printf "Merging pot files to create: e2m3u2bouquet.pot\n"
cat e2m3u2bouquet-py.pot e2m3u2bouquet-xml.pot | msguniq -s --no-wrap --no-location -o e2m3u2bouquet.pot -
#printf "remove pot Creation date\n"
#$localgsed -i -e'/POT-Creation/d' e2m3u2bouquet.pot
printf "remove temp pot files\n"
rm e2m3u2bouquet-py.pot e2m3u2bouquet-xml.pot
